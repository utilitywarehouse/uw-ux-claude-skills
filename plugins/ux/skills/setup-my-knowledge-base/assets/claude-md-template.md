...
- To release any change to `uw-ux-claude-skills` — a new skill, an edit to an existing one, retiring one, or even a docs-only wording change like the site's index page — use the `contribute-to-skills-repo` skill. It opens a PR against `uw-ux-claude-skills` and stops there; it never merges, since that repo's `main` needs review and sign-off from whoever owns it. Never commit straight to `main` in this repo, even for a one-line edit.
