...
Sources worth checking when a defect looks systemic: the `new-project-setup` skill's embedded CLAUDE.md template, the `research-transcript-cleaner` and `transcript-coder` frontmatter templates, the Obsidian Web Clipper's property template, and Obsidian's own settings.
