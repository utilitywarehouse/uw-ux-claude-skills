---
name: research-transcript-cleaner
version: 3
description: Clean video transcripts from UX research sessions with participants — moderated interviews, usability tests, and unmoderated test recordings — into a coding-ready markdown file for your knowledge base. Use this skill whenever the user provides a raw transcript of a research session (pasted text, uploaded file, or referenced from Drive/Dovetail) and wants it tidied up for analysis. Trigger on phrases like "clean this transcript", "tidy up this session", "prep this for coding", "remove the fillers", "intelligent verbatim", or when they upload a .vtt, .srt, .txt, .docx, or messy interview text from a research session and asks for it to be made readable. Also trigger when they share a Userlytics, Dovetail, Google Meet, Zoom, or Teams transcript export of an interview or usability test with any framing that suggests it's headed for thematic analysis — even if they don't say the word "clean". Do NOT use this for internal team meetings, standups, planning calls, or any transcript without a research participant — those aren't coded for themes and don't fit this workflow's cohort/session-type/speaker-labelling questions; clean those by hand instead, following the project's own meeting-notes convention. When in doubt, ask whether the recording involved a research participant before triggering.
---
