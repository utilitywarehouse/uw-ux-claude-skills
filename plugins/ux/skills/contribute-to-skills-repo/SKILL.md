---
name: contribute-to-skills-repo
version: 7
description: Ship a finished change to uw-ux-claude-skills — a new skill, an edit to an existing one, retiring one, or any other change to the repo (docs, config, wording) — into a pull request for repo admins to review and merge. Use this whenever a team member has just finished writing a new skill (typically with skill-creator), editing an existing one here, decided a skill should be removed, or made any other change to the repo and wants to submit it. Trigger on phrases like "propose this skill", "contribute this change", "submit my skill", "ship this to the team repo", "send this for review", "open a PR for this skill", "retire this skill", "remove this skill", or "how do I get this merged". This skill NEVER merges anything itself — main is protected and only repo admins approve merges. It runs this repo's smoke test for the skill before every push, but does not package a .skill file or touch Cowork — this repo ships purely via GitHub pull request, nothing else.
---
